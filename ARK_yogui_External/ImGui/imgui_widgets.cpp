...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (*v)
    {
       ...
       // RenderCheckMark(check_bb.Min + ImVec2(pad, pad), GetColorU32(ImGuiCol_CheckMark), square_sz - pad*2.0f);
		ImVec2 CheckMarkPos = pos;
		CheckMarkPos.x += 6; CheckMarkPos.y += 6;
		ImRect CheckMark_bb(CheckMarkPos, CheckMarkPos + ImVec2(square_sz - 12, square_sz - 12));
		RenderFrame(CheckMark_bb.Min, CheckMark_bb.Max, GetColorU32(ImGuiCol_CheckMark), false);
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
	ImVec2 FrameMinSize = window->DC.CursorPos;
	//FrameMinSize.y -= ;
    ImRect frame_bb(FrameMinSize, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y*2.0f + label_size.y / 2));
    ...
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
	ImVec2 FrameMin = frame_bb.Min; FrameMin.y += label_size.y + style.FramePadding.y;
	ImVec2 FrameMax = frame_bb.Max; FrameMax.y += label_size.y + style.FramePadding.y; FrameMax.y -= 20.f;
    RenderFrame(FrameMin, FrameMax, frame_col, true, g.Style.FrameRounding);

	ImRect SliderFrameRect(FrameMin, FrameMax);

    // Slider behavior
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(SliderFrameRect, id, data_type, v, v_min, v_max, format, power, ImGuiSliderFlags_None, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);
    ...
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, v, format);
	ImVec2 ClippedMin = ImVec2(frame_bb.Max.x, frame_bb.Min.y + 5);
	ImVec2 ClippedMax = ImVec2(frame_bb.Max.x + 25.f, frame_bb.Max.y);
    RenderTextClipped(ClippedMin, ClippedMax, value_buf, value_buf_end, NULL, ImVec2(0.5f,0.5f));
...
bool ImGui::CostomSliderScalar(ImFont * pVarFont, const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems || !pVarFont)
		return false;
	...
	ImVec2 FrameMinSize = window->DC.CursorPos;
	//FrameMinSize.y -= ;
	ImRect frame_bb(FrameMinSize, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y*2.0f + label_size.y / 2 + 5.f));
	ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
	...
	const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
	RenderNavHighlight(frame_bb, id);
	ImVec2 FrameMin = frame_bb.Min; FrameMin.y += label_size.y + style.FramePadding.y + 2.5f;
	ImVec2 FrameMax = frame_bb.Max; FrameMax.y += label_size.y + style.FramePadding.y; FrameMax.y -= 22.5f;
	RenderFrame(FrameMin, FrameMax, frame_col, true, g.Style.FrameRounding);
	...
	ImVec2 GrabCenter(grab_bb.Min.x + (grab_bb.Max.x - grab_bb.Min.x) / 2, grab_bb.Min.y + (grab_bb.Max.y - grab_bb.Min.y) / 2);

	// Render grab
	if (grab_bb.Max.x > grab_bb.Min.x)
		window->DrawList->AddCircleFilled(GrabCenter, 4.f, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab));
		//window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);

	ImGui::PushFont(pVarFont);
	// Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
	char value_buf[64];
	const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, v, format);
	ImVec2 ClippedMin = ImVec2(frame_bb.Max.x + 1.f, frame_bb.Min.y + 9);
	ImVec2 ClippedMax = ImVec2(frame_bb.Max.x + 40.f, frame_bb.Max.y);
	RenderTextClipped(ClippedMin, ClippedMax, value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
	ImGui::PopFont();
	if (label_size.x > 0.0f)
		RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y), label);
...
IMGUI_API bool ImGui::CostomSliderFloat(ImFont * pFont, const char * label, float * v, float v_min, float v_max, const char * format, float power)
{
	return CostomSliderScalar(pFont,label, ImGuiDataType_Float, v, &v_min, &v_max, format, power);
}
...
IMGUI_API bool ImGui::CustomSliderInt(ImFont * pFont, const char * label, int * v, int v_min, int v_max, const char * format)
{
	return CostomSliderScalar(pFont, label, ImGuiDataType_S32, v, &v_min, &v_max, format, 1);
}
