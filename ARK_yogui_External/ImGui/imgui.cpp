...
ImGuiIO::ImGuiIO()
{
    ...
    IniSavingRate = 5.0f;
    IniFilename = "VerbalComms.ini";
    LogFilename = "VerbalComms_log.txt";
...
{
    bool error = false;
    if (strcmp(version, IMGUI_VERSION)!=0) { error = true; IM_ASSERT(strcmp(version,IMGUI_VERSION)==0 && "Error vers");  }
    if (sz_io    != sizeof(ImGuiIO))       { error = true; IM_ASSERT(sz_io    == sizeof(ImGuiIO)      && "Error struct"); }
    if (sz_style != sizeof(ImGuiStyle))    { error = true; IM_ASSERT(sz_style == sizeof(ImGuiStyle)   && "Error struct"); }
    if (sz_vec2  != sizeof(ImVec2))        { error = true; IM_ASSERT(sz_vec2  == sizeof(ImVec2)       && "Error struct"); }
    if (sz_vec4  != sizeof(ImVec4))        { error = true; IM_ASSERT(sz_vec4  == sizeof(ImVec4)       && "MError struct"); }
    if (sz_vert  != sizeof(ImDrawVert))    { error = true; IM_ASSERT(sz_vert  == sizeof(ImDrawVert)   && "Error struct"); }
    return !error;
}
...
ImGuiIO& ImGui::GetIO()
{
    IM_ASSERT(GImGui != NULL && "No Instance");
    return GImGui->IO;
}
...
void ImGui::NewFrame()
{
    ...
    IM_ASSERT((g.IO.DeltaTime > 0.0f || g.FrameCount == 0)              && "Need + De");
...
static bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    char title[256];
    if (name)
        ImFormatString(title, IM_ARRAYSIZE(title), "%s %s %0xllX", parent_window->Name, name, id);
    else
        ImFormatString(title, IM_ARRAYSIZE(title), "%s %0xllX", parent_window->Name, id);
...
static void CheckStacksSize(ImGuiWindow* window, bool write)
{
    ...
    { int current = window->IDStack.Size;       if (write) *p_backup = (short)current; else IM_ASSERT(*p_backup == current && "");   p_backup++; }    // Too few or too many PopID()/TreePop()
    { int current = window->DC.GroupStack.Size; if (write) *p_backup = (short)current; else IM_ASSERT(*p_backup == current && "");                p_backup++; }    // Too few or too many EndGroup()
    { int current = g.BeginPopupStack.Size;     if (write) *p_backup = (short)current; else IM_ASSERT(*p_backup == current && ""); p_backup++;}// Too few or too many EndMenu()/EndPopup()
    // For color, style and font stacks there is an incentive to use Push/Begin/Pop/.../End patterns, so we relax our checks a little to allow them.
    { int current = g.ColorModifiers.Size;      if (write) *p_backup = (short)current; else IM_ASSERT(*p_backup >= current && "");       p_backup++; }    // Too few or too many PopStyleColor()
    { int current = g.StyleModifiers.Size;      if (write) *p_backup = (short)current; else IM_ASSERT(*p_backup >= current && "");           p_backup++; }    // Too few or too many PopStyleVar()
    { int current = g.FontStack.Size;           if (write) *p_backup = (short)current; else IM_ASSERT(*p_backup >= current && "");                   p_backup++; }    // Too few or too many PopFont()
    ...
    if (first_begin_of_the_frame)
    {
        ...
        if (!(flags & ImGuiWindowFlags_NoTitleBar))
        {
            ...
            if (p_open != NULL)
            {
                ...
                if (CloseButton(window->GetID(XorString("#CLOSE")), ImVec2(window->Pos.x + window->Size.x - style.FramePadding.x - rad, window->Pos.y + style.FramePadding.y + rad), rad + 1))
                    *p_open = false;
            }
...
const char* ImGui::GetStyleColorName(ImGuiCol idx)
{
    ...
    switch (idx)
    {
	case ImGuiCol_Text: return XorString("Text");
	case ImGuiCol_TextDisabled: return XorString("TextDisabled");
	case ImGuiCol_WindowBg: return XorString("WindowBg");
	case ImGuiCol_ChildBg: return XorString("ChildBg");
	case ImGuiCol_PopupBg: return XorString("PopupBg");
	case ImGuiCol_Border: return XorString("Border");
	case ImGuiCol_BorderShadow: return XorString("BorderShadow");
	case ImGuiCol_FrameBg: return XorString("FrameBg");
	case ImGuiCol_FrameBgHovered: return XorString("FrameBgHovered");
	case ImGuiCol_FrameBgActive: return XorString("FrameBgActive");
	case ImGuiCol_TitleBg: return XorString("TitleBg");
	case ImGuiCol_TitleBgActive: return XorString("TitleBgActive");
	case ImGuiCol_TitleBgCollapsed: return XorString("TitleBgCollapsed");
	case ImGuiCol_MenuBarBg: return XorString("MenuBarBg");
	case ImGuiCol_ScrollbarBg: return XorString("ScrollbarBg");
	case ImGuiCol_ScrollbarGrab: return XorString("ScrollbarGrab");
	case ImGuiCol_ScrollbarGrabHovered: return XorString("ScrollbarGrabHovered");
	case ImGuiCol_ScrollbarGrabActive: return XorString("ScrollbarGrabActive");
	case ImGuiCol_CheckMark: return XorString("CheckMark");
	case ImGuiCol_SliderGrab: return XorString("SliderGrab");
	case ImGuiCol_SliderGrabActive: return XorString("SliderGrabActive");
	case ImGuiCol_Button: return XorString("Button");
	case ImGuiCol_ButtonHovered: return XorString("ButtonHovered");
	case ImGuiCol_ButtonActive: return XorString("ButtonActive");
	case ImGuiCol_Header: return XorString("Header");
	case ImGuiCol_HeaderHovered: return XorString("HeaderHovered");
	case ImGuiCol_HeaderActive: return XorString("HeaderActive");
	case ImGuiCol_Separator: return XorString("Separator");
	case ImGuiCol_SeparatorHovered: return XorString("SeparatorHovered");
	case ImGuiCol_SeparatorActive: return XorString("SeparatorActive");
	case ImGuiCol_ResizeGrip: return XorString("ResizeGrip");
	case ImGuiCol_ResizeGripHovered: return XorString("ResizeGripHovered");
	case ImGuiCol_ResizeGripActive: return XorString("ResizeGripActive");
	case ImGuiCol_Tab: return XorString("Tab");
	case ImGuiCol_TabHovered: return XorString("TabHovered");
	case ImGuiCol_TabActive: return XorString("TabActive");
	case ImGuiCol_TabUnfocused: return XorString("TabUnfocused");
	case ImGuiCol_TabUnfocusedActive: return XorString("TabUnfocusedActive");
	case ImGuiCol_PlotLines: return XorString("PlotLines");
	case ImGuiCol_PlotLinesHovered: return XorString("PlotLinesHovered");
	case ImGuiCol_PlotHistogram: return XorString("PlotHistogram");
	case ImGuiCol_PlotHistogramHovered: return XorString("PlotHistogramHovered");
	case ImGuiCol_TextSelectedBg: return XorString("TextSelectedBg");
	case ImGuiCol_DragDropTarget: return XorString("DragDropTarget");
	case ImGuiCol_NavHighlight: return XorString("NavHighlight");
	case ImGuiCol_NavWindowingHighlight: return XorString("NavWindowingHighlight");
	case ImGuiCol_NavWindowingDimBg: return XorString("NavWindowingDimBg");
	case ImGuiCol_ModalWindowDimBg: return XorString("ModalWindowDimBg");
    }
    IM_ASSERT(0);
    return "Unknown";
}
...
bool ImGui::BeginPopupEx(ImGuiID id, ImGuiWindowFlags extra_flags)
{
    ...
    if (extra_flags & ImGuiWindowFlags_ChildMenu)
        ImFormatString(name, IM_ARRAYSIZE(name), XorString("##Mem %02d"), g.BeginPopupStack.Size); // Recycle windows based on depth
    else
        ImFormatString(name, IM_ARRAYSIZE(name), XorString("##Pop %08x"), id); // Not recycling, so we can close/open during the same frame
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags)
{
    ...
    {
        window = NULL;
        source_id = ImHashStr(XorString("#SourceExtern"));
...
void ImGui::LogButtons()
{
    ...
    PushID(XorString("LogButtons"));
    const bool log_to_tty = Button(XorString("Log To TTY")); SameLine();
    const bool log_to_file = Button(XorString("Log To File")); SameLine();
    const bool log_to_clipboard = Button(XorString("Log To Clipboard")); SameLine();
    PushAllowKeyboardFocus(false);
    SetNextItemWidth(80.0f);
    SliderInt(XorString("Default Depth"), &g.LogDepthToExpandDefault, 0, 9, NULL);
